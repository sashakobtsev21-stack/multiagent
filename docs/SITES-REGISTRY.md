...
| Privacy/cookie-policy | ✅ `/privacy/` (2026-06-30) | ✅ | ✅ | ✅ | ✅ |
...
3 новых форка движка, EN-first: собраны, проходят **полный `npm run qa` (ВЕРДИКТ GO)**, запушены в публичные репо, но **без контента и без живого домена** (не индексируются). У всех есть страница privacy/cookie-policy (единый порт по сети 2026-06-30). В `/work`-ротацию НЕ входят до запуска (память `new-sites-tech-ready-before-content`). Подключены к дашборду (grafana). Планы — `docs/new-sites/{country}-plan-2026-06-30.md`; SEO — `docs/seo/{country}-seo-2026-06-30.md`.

> **Поправка 2026-06-30:** при добавлении privacy-страницы вскрылось, что armenia/serbia на скаффолде были провалидированы только `build`/`test`, но НЕ полным `qa` — и фактически были NO-GO: у armenia висела ссылка `/rss.xml` без endpoint, у serbia — незавершённая en-only миграция (14 TS-ошибок `'ru'→'en'` + ru-hreflang). Оба дефекта исправлены тем же заходом; теперь все 3 реально `qa`-GO. Урок зафиксирован в памяти `new-sites-tech-ready-before-content` (валидировать полным `qa`, не только build).
