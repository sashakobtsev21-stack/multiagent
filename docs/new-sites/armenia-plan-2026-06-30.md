...
**Языковая модель: EN-first (на старте — только английский).**
- `LANGS = ['en']` на старте, как macedonia/albania (en на корне `/`, без `/ru/`, без uk). Донор копии — **`cp -r sites/macedonia-site`** (en-only форк уже несёт en-категории, Consent-паттерн, без uk).
- **ru-кластер релокации — фаст-фоллоу ОТДЕЛЬНЫМ треком, НЕ на старте.** Подключаем после того, как EN-ядро наберёт траст и дисплей-порог. Тогда переход на `LANGS = ['en','ru']` (паритет slug, hreflang en↔ru, x-default→en). **⚠️ ВАЖНО (см. §4): ru-кластер пока НЕ доходный — у нас нет ни одного рабочего партнёра под релокацию/банки. Планировать его как траст-трафик, не как деньги, до заведения реального affiliate.**
- Армения уникальна тем, что ru — де-факто второй язык страны и огромный эвергрин-спрос ru-релокации; но монетизация этого спроса у нас сегодня не закрыта. Поэтому ru — приоритетный фаст-фоллоу по трафику, но с честной пометкой про деньги.
